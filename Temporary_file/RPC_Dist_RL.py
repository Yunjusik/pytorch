...
AGENT_NAME = "agent"
OBSERVER_NAME = "observer{}" ## observer makes its own environment, wait for agent's command to run an episode
...
## Rref -> remote reference --> serve as distributed shared pointer to a local or remote object
## each RRef only has one owner and the object only lives on that owner,
## In DDRL, RRef plays a role with distributed prioritized experience replay...
...
class Observer:
    ...
    def run_episode(self, agent_rref, n_steps):
        ...
        """
        state, ep_reward = self.env.reset(), 0  # episode reward is initialized to zero.
...
class Agent:
    def __init__(self, world_size):
        ...
        self.saved_log_probs = {}
        self.policy = Policy()
        #self.optimizer = optim.RMSprop(self.policy.parameters(), lr=1e-2)
    ...
    def select_action(self, ob_id, state):
        ...
        #print(probs)
        m = Categorical(probs)
        action = m.sample() # m -> [probability matrix] --> then, sampling [1] for 90% chance
        #print(action)
